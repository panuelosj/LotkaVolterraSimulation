...
void ecoRun(int** simEnv, int** simEnvAge, int simLocal[], float aBirthRate, float predationRates[], float deathRates[]){
	...
	if (state == 0){
		for (rate = aBirthRate; rate > 0; rate--) {
			x = (rand() % ENV_SIZE_X);
			y = (rand() % ENV_SIZE_Y);
	...
	// Specialist Predation (state=prey trophic level)
		...
		for (rate = predationRates[state-1]; rate > 0; rate--){
			...
			if (state == 1 && simLocal[0] == 1 && simLocal[loc2] == 2 && (rand()/(float)RAND_MAX) < rate) {
				simEnv[x][y] = 2;
				simEnvAge[x][y] = 0;
			}
			else if (state == 2 && simLocal[0] == 1 && simLocal[loc2] == 3 && (rand()/(float)RAND_MAX) < rate) {
				simEnv[x][y] = 3;
				simEnvAge[x][y] = 0;
			}
		}
